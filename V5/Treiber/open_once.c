...
#define DEV_CLASS_NAME "c_open_once"
#define DEV_DRIVER "open_once_driver"
...
static int open_once_open(struct inode *i, struct file *instance)
{
...
static int open_once_close(struct inode *i, struct file *instance)
{
...
static ssize_t open_once_read(struct file *instance, char __user *buf, size_t len, loff_t *off)
{
	printk(KERN_INFO "%s: read()\n", DEV_DRIVER);
	return 0;
}

static ssize_t open_once_write(struct file *instance, const char __user *buf, size_t len, loff_t *off)
{
...
static struct file_operations fops =
{
	.owner = THIS_MODULE,
	.open = open_once_open,
	.release = open_once_close,
	.read = open_once_read,
	.write = open_once_write	
};

static int __init open_once_init(void)
{
...
static void __exit open_once_exit(void)
{
...
module_init(open_once_init);
module_exit(open_once_exit);
